...
Experiments were conducted \textit{in silico} using stochastc agent-based model approaches.
...
To assess the generality of our findings, we supplemented work with a simple counter-based ``Poisson'' genome model drawn from \cite{raynes2018sign}  with an alternate ``Site-explicit'' genome model.

This section reviews the model mechanics used across experiments in this work, as well aspects of simulation design that we manipulated between treatment conditions.
Explored dimensions include population size, spatial population structure, background prevalence of hypermutator traits, and restriction of adative potential.
...
Mutations are assumed to have interchangle fitness impacts, so
a genome's fitness score is calculated simply as $n_b - n_d$.
selection outcomes is described in section \ref{sec:evolution}.

A mutation processs is applied to agent genomes each generation.
...
In some experiments where adaptive potential was limited, we imposed a upper bound on the total quantity of beneficial mutatious that could be accrued.
...
Consistent with prior work \citep{raynes2018sign}, we conducted experiments initialized with an even mix of normo- and hyper-mutators;
agents in the founding population were randomly assigned hyper- or normo-mutator status, with equal probability.
...
A key implicit assumption of Poisson genome model, in its original formulation, is the availability of unlimited adaptive potential.
...
Each loci has probability $p_b = 10^{-6}$ of transitioning $0 \rightarrow 1$ when inherited from parent to offspring.
...
Site-explicit hypermutator mechanics also closely follow those under the Poisson model.
...
Under default conditions, demes were arranged as two-dimensional grid lattice with non-toroidal boundaries (i.e., grid edges did not wrap around).
...
Hence, we chose to scale edge width osurveyed population sizes by factors of 9: $1 \times 1 \times 256,\;\; 3 \times 3 \times 256,\;\; 9 \times 9 \times 256,\;\; \ldots,\;\; 729 \times 729 \times 256$.
...
aHence, replicate count decreased with population size, but was boosted to at least 9 replicates in all cases.
...
These architecture of these hardware accelerators comprise large collections of worker processors specialized for numerical computation.
...
In some cases, experiments on WSE were extended beyond the $729 \times 729$ to fill the CS-2 WSE's entire $750 \times 994$ compliment of available PEs.
...
At full wafer scale, our largest population net population size reached 1.5 billion agents.
...
Our WSE simulations relied on Cerebras Software Language's PRNG utilies, which use a Linear-Feedback Shift Register (LFSR) approach with polynomial $3x^2 + 8x + 1$ strided by 128 iterations.
...
This PRNG also applies a LSFR approach \citep{brent2004note}.

PRNG patterning, particularly lower-quality Linear Congruence Generator (LCG) algorithms, has been shown in some cases to substantially bias results of \citep{click2010quality}.
...
The anomoly occured solely under the site-explicit genome model, where the number of PRNG calls made to apply mutation to an agent differs with the quantity of available beneficial mutations.
...
Fully re-running the affected simulation did not resolve the data anomoly.
...
As such, for one data point, we exclude the originally-generated data in favor of the subsequent PRNG no-op padded results.
...
To assess the efficacy hardware accelerator resources in expediting our simulations, we collected timings of execution duration from our simulation experiments.
